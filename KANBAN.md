# Kanban Board

<!-- Format definitions: See .claude/rules/task_format.md -->

## Backlog

- [ ] **[UX]** Add web chat interface for users (similar to open-webui) (P2)
- [ ] **[UX]** Create Electron desktop application (P2)
- [ ] **[ARCHITECTURE]** Implement NLP style analyzer (TF-IDF, N-Grams, linguistic metrics) (P1)
- [ ] **[UX]** Add document source import (PDF, MD, TXT) (P1)
- [ ] **[ARCHITECTURE]** Create automatic persona prompt generation from style analysis (P1)
- [ ] **[UX]** Build Admin Dashboard (React + Vite + Tailwind + Shadcn) (P2)
- [ ] **[UX]** Implement source management API (CRUD endpoints) (P2)
- [ ] **[SECURITY]** Add regex-based data anonymization for sensitive content (P2)
- [ ] **[TEST]** Write comprehensive E2E tests for proxy functionality (P3)
- [ ] **[DEVOPS]** Implement real-time logging system with WebSocket (P3)
- [ ] **[UX]** Add multi-profile style management support (P3)
- [ ] **[CONFIG]** Create CLI for configuration management (P3)
- [ ] **[UX]** Build web chat interface (Open WebUI-style) for user interaction (P2)
- [ ] **[UX]** Create Electron desktop application wrapper (P2)

## In Progress

<!-- Tasks in progress appear here -->
