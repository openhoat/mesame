...
export function ConversationHistory({
  ...
  async function handleConvertToSource(id: string, event: React.MouseEvent) {
    event.stopPropagation()
    if (!window.confirm('Add this conversation to sources?')) return

    try {
      setError(null)
      await convertConversationToSource(id)
      // Show success message
      alert('Conversation added to sources successfully!')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to convert conversation to source')
    }
  }
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        type="button"
                        onClick={e => handleConvertToSource(conv.id, e)}
                        className="text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 p-1"
                      ...
                      >
                        <svg
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                          role="img"
                          aria-label="Add to sources icon"
                        >
                          <title>Add to sources</title>
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={2}
